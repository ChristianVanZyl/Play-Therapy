<!--  Footer code inserted below-->
<div class="container-fluid p-4" id="footerContainer">
  <div class="row align-items-center">
    <div class="col-sm-4 pt-4 text-center" id="footerRow">
      <table class="table table-borderless" id="footerTable">
        <thead>
          <tr>
            <th scope="col">Contact us</th>
            <th scope="col">Information</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>(+27) 072 135 8454</td>
            <td><a href="aboutUs.php" style="text-decoration: none; color: white;">About Us</a></td>
          </tr>
          <tr>
            <td>Address:</td>
            <td><a href="FAQ.php" style="text-decoration: none; color: white;">FAQ</a></td>
          </tr>
          <tr>
            <td>Steynsrust</td>
            <td><a href="privacyPolicy.php" style="text-decoration: none; color: white;">Privacy Policy</a></td>
          </tr>
          <tr>
            <td>Somerset West</td>
            <td><a href="termsandconditions.php" style="text-decoration: none; color: white;">Terms and Conditions</a></td>
          </tr>
          <tr>
            <td>7130</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="col-sm-4 text-center" id="payfastCol">
      <img class="img-fluid" src="./images/payfast.png" alt="payfast" id="payfastImg">
    </div>
    <div class="col-sm-4 text-center" id="socialMediaIcons">
      <a href="https://playtherapies.co.za/services/" class="servicesIcon" id="socialMediaImg"><img class="imgPlay" src="./images/picture2.png" style="width: 100px;" alt="services"></a>
      <a href="https://www.facebook.com/LenkaDeVilliersVanZylPlayTherapy/" class="social-icon fab fa-facebook-f fa-2x p-4" id="socialMediaImg"></a>
      <a href="https://www.linkedin.com/in/lenka-de-villiers-van-zyl-325768120/" class="social-icon fab fa-linkedin fa-2x p-4" id="socialMediaImg"></a>
      <a href="contactUs.php" class="social-icon fas fa-envelope fa-2x p-4" id="socialMediaImg"></a>
    </div>
  </div>
</div>
<p class="copyrightText"> &#169; Play Therapy Toys by Lenka De Villiers-Van Zyl</p>
